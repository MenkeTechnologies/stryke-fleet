...
|---|--------|------|----:|------------|
| 1 | `Fleet::Session` | `lib/Session.stk` | 19 | `open` · `send` · `send_line` · `send_secret` (transmit but redact) · `send_control` (Ctrl-C/D/Z, pexpect sendcontrol) · `expect` · `branch` · `read` · `buffer` · `alive` · `eof` · `interact` · `close` · `transcript` · `events` · `matches` · `last_match` · `timed_out` · `transcript_text` |
...
| 3 | `Fleet::Recipes` | `lib/Recipes.stk` | 41 | `ssh_login` · `ssh_copy_id` · `sudo` · `su` · `psql` · `mysql` · `redis_cli` · `mongo` · `yes_to_all` · `cisco_enable` · `telnet_login` · `sftp` · `scp` · `ftp_login` · `passwd_change` · `docker_login` · `npm_login` · `gpg_decrypt` · `openssl_pem` · `rsync` · `ssh_keygen` · `git_clone` · `kinit` · `ldapsearch` · `restic` · `borg` · `smbclient` · `vault_login` · `aws_configure` · `htpasswd` · `keytool` · `ansible_vault` · `sqlplus` · `op_signin` · `ssh_add` · `unzip_encrypted` · `cqlsh` · `clickhouse_client` · `gcloud_auth` · `heroku_login` · `names` |
| 4 | `Fleet::Fanout` | `lib/Fanout.stk` | 10 | `run` (N targets, `pmap`) · `batch` (bounded concurrency, K at a time) · `ssh` (host-list convenience) · `partition` · `summarize` (counts + error lines) · `group_by_error` (bucket failures by error) · `group_by_output` (bucket successes by output — config-drift) · `grep_transcript` (which hosts printed X) · `retry_failed` (re-run only the failures) · `retry_until` (re-run failures up to N times) |
