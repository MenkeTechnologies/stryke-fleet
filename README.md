...
| 4 | `Fleet::Fanout` | `lib/Fanout.stk` | 9 | `run` (N targets, `pmap`) · `batch` (bounded concurrency, K at a time) · `ssh` (host-list convenience) · `partition` · `summarize` (counts + error lines) · `group_by_error` (bucket failures by error) · `grep_transcript` (which hosts printed X) · `retry_failed` (re-run only the failures) · `retry_until` (re-run failures up to N times) |
