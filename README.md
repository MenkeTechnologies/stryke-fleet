...
| 3 | `Fleet::Recipes` | `lib/Recipes.stk` | 40 | `ssh_login` · `ssh_copy_id` · `sudo` · `su` · `psql` · `mysql` · `redis_cli` · `mongo` · `yes_to_all` · `cisco_enable` · `telnet_login` · `sftp` · `scp` · `ftp_login` · `passwd_change` · `docker_login` · `npm_login` · `gpg_decrypt` · `openssl_pem` · `rsync` · `ssh_keygen` · `git_clone` · `kinit` · `ldapsearch` · `restic` · `smbclient` · `vault_login` · `aws_configure` · `htpasswd` · `keytool` · `ansible_vault` · `sqlplus` · `op_signin` · `ssh_add` · `unzip_encrypted` · `cqlsh` · `clickhouse_client` · `gcloud_auth` · `heroku_login` · `names` |
