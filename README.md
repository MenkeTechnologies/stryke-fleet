...
| 4 | `Fleet::Fanout` | `lib/Fanout.stk` | 7 | `run` (N targets, `pmap`) · `batch` (bounded concurrency, K at a time) · `ssh` (host-list convenience) · `partition` · `summarize` (counts + error lines) · `group_by_error` (bucket failures by error) · `retry_failed` (re-run only the failures) |
