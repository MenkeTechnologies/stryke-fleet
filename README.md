...
|---|--------|------|----:|------------|
| 1 | `Fleet::Session` | `lib/Session.stk` | 21 | `open` · `send` · `send_line` · `send_lines` (one line per `send` event) · `send_secret` (transmit but redact) · `send_control` (Ctrl-C/D/Z, pexpect sendcontrol) · `expect` · `expect_any` (which of several prompts arrived) · `branch` · `read` · `buffer` · `alive` · `eof` · `interact` · `close` · `transcript` · `events` · `matches` · `last_match` · `timed_out` · `transcript_text` |
...
| 4 | `Fleet::Fanout` | `lib/Fanout.stk` | 14 | `run` (N targets, `pmap`) · `batch` (bounded concurrency, K at a time) · `ssh` (host-list convenience) · `partition` · `count` (total/ok/failed tally) · `all_ok` (boolean fleet-pass gate) · `failed_cmds` (cmd strings of failures) · `pluck` (project one key per result) · `summarize` (counts + error lines) · `group_by_error` (bucket failures by error) · `group_by_output` (bucket successes by output — config-drift) · `grep_transcript` (which hosts printed X) · `retry_failed` (re-run only the failures) · `retry_until` (re-run failures up to N times) |
